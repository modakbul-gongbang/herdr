...
herdr agent prompt <target> <text> [--expected-input-guard TOKEN] [--wait] [--until STATUS]... [--timeout MS]
...
Pass the `input_guard` returned by `agent get` as `--expected-input-guard` when delivery must remain pinned to that detected agent process.
This selects the separate guarded socket operation, which validates ownership before any bytes and again before delayed Enter.
Mismatch returns `agent_input_guard_mismatch` without input.
A change after body delivery returns `partial`, suppresses Enter, and exits with status 1.
Unsupported older servers reject guarded delivery and Herdr does not fall back to a plain prompt.
