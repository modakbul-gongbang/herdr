...
For guarded delivery, read `.result.agent.input_guard` from `agent get`, then pass it as `--expected-input-guard` on `agent prompt`.
The guard is stable while the same detected agent process moves between working and idle, and rotates when Herdr observes a release, exit, relaunch, or replacement.
Herdr validates the guard in the PTY writer before focus bytes or prompt text and again before the delayed Enter.
A mismatch writes nothing and returns `agent_input_guard_mismatch`.
If the target changes after the body is written but before Enter, Herdr suppresses Enter, returns `outcome: "partial"`, and the CLI exits with status 1.
This is an observed lifecycle guarantee, not a kernel-level guarantee for a process change Herdr cannot observe.
Older servers reject the separate `agent.prompt_guarded` operation; callers must not retry it as `agent.prompt`.
...
Successful `agent start`, `agent prompt`, and `agent wait` commands return the current agent at `.result.agent`.
Guarded prompt responses also return the exact requested target, expected guard, and `submitted` or `partial` outcome.
`pane wait-output` returns `.result.pane_id`, `.result.matched_line`, and the matched snapshot at `.result.read`.
...
guard=$(herdr agent get reviewer | jq -r '.result.agent.input_guard')
herdr agent prompt reviewer "Apply the approved change" --expected-input-guard "$guard"
